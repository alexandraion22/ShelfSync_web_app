...
import { Typography, FormControlLabel, Slider, Radio, TextField, Rating, Box, Table, TableBody, TableCell, TableContainer, TablePagination, TableRow, Paper, IconButton, Stack, Select, MenuItem } from '@mui/material';
import axios from 'axios';
import SearchIcon from '@mui/icons-material/Search';
import { useNavigate } from 'react-router-dom';

const CurrentlyReadingReader = ({authUser, token}) => {
    ...
    const handleSliderChange = (id, value) => {
        const updatedFeedbackList = feedbackList.map(feedback => {
            if (feedback.id === id) {
              return { ...feedback, current_progress: value };
            }
            return feedback;
          });
    ...
    const handleSliderChangeCommitted = (id, value) => {
    // Make an Axios call to update the progress in the backend
    axios.put("http://localhost:8083/list_elements/updateElementById", {
        element_id: feedbackList[id].listElement_id,
        current_pages: value,
        progress: feedbackList[id].progress
      }, config)
      .then(response => {
    ...
    useEffect(() => {
        ...
        .then ((result) => {
        const feedbacksWithId = result.data.filter(feedback => feedback.progress === "cr").map((feedback, index) => {
                            ...
                            <TableBody sx={{ width: '100%', mb: 2, borderRadius: 5}}>
                                ...
                                return (
                                    ...
                                    <TableCell align="center" sx={{ textAlign: 'center' }}>
                                        <Typography variant="body1" sx={{ marginBottom: 1 }}>{row.current_progress}/{row.pages}</Typography>
                                        <Slider
                                            value={row.current_progress} // Set the value of the Slider to the current pages
                                            min={0} // Set the minimum value
                                            max={row.pages} // Set the maximum value to the total pages
                                            onChange={(event, value) => handleSliderChange(row.id, value)} // Handle the onChange event
                                            onChangeCommitted={(event, value) => handleSliderChangeCommitted(row.id, value)} // Call handleSliderChangeCommitted when the user finishes adjusting the slider
                                            sx={{ width: '30em', margin: 'auto', color: '#60A494'}} // Set the width and margin of the Slider
                                        />
                                    </TableCell>
...
export default CurrentlyReadingReader;
