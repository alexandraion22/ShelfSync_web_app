...
const WantToReadReader = ({authUser, token}) => {
    ...
    useEffect(() => {
        ...
        .then ((result) => {
        const feedbacksWithId = result.data.filter(feedback => feedback.progress === "wtr").map((feedback, index) => {
                            ...
                            <div>
                                {Array.from({ length: 5 }, (_, index) => (
                                    <FormControlLabel
                                        ...
                                        label={<Rating name="read-only" value={index + 1} readOnly size="small" style={{transform: 'scale(1.2)'}}/>}
                                        labelPlacement="end"
                                    />
                                ))}
                            </div>
...
export default WantToReadReader;
